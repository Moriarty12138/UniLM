        ...
        if file_oracle is None:
            with open(file_src, "r", encoding='utf-8') as f_src, open(file_tgt, "r", encoding='utf-8') as f_tgt:
                for src, tgt in zip(f_src, f_tgt):
                    src_tk = tokenizer.tokenize(src.strip())  # 分词 tokenizer.tokenize
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __init__(self, max_pred, mask_prob, vocab_words, indexer, max_len=512,
                skipgram_prb=0, skipgram_size=0, block_mask=False,
                mask_whole_word=False, new_segment_ids=False,
                truncate_config={}, mask_source_words=False, mode="s2s",
                has_oracle=False, num_qkv=0, s2s_special_token=False,
        ...
        self.indexer = indexer  # function from token to token index  传入的是 tokenizer.convert_tokens_to_ids
        self.max_len = max_len
    ...
    def __call__(self, instance):
        tokens_a, tokens_b = instance[:2]  # 句子 A 和句子 B，在这一步就已经完成了分词

        if self.pos_shift:
            tokens_b = ['[S2S_SOS]'] + tokens_b

        # -3  for special tokens [CLS], [SEP], [SEP]
        num_truncated_a, _ = truncate_tokens_pair(  # 截断
            tokens_a, tokens_b,
            self.max_len - 3,
