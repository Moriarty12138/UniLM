...
def main():
    ...
    parser.add_argument("--model_recover_path", default=None, type=str, required=False,  # 进行微调的预训练模型
    ...
    parser.add_argument('--tokenized_input', action='store_true',  # 如果已经做好了分词，直接按照空格进行分词
    ...
    # assert Path(args.model_recover_path).exists(), "--model_recover_path doesn't exist"  # 预训练模型必须存在
    ...
    if args.do_train:
        ...
        bi_uni_pipeline = [seq2seq_loader.Preprocess4Seq2seq(
            ...
            tokenizer.convert_tokens_to_ids,  # 直接将 token 转换为 id ，但是在处理中文时会存在无法完全切分开的问题
            args.max_seq_length,
        ...
        train_dataset = seq2seq_loader.Seq2SeqDataset(
            fn_src, fn_tgt, args.train_batch_size,
            data_tokenizer, args.max_seq_length,
